...
object MainInMemoryLogging extends ServerApp {
  ...
  val srv: RaceRegistrationService[Log[Pure, ?]] =
    new RaceRegistrationService[Log[Pure, ?]](new (Log[Pure, ?] ~> Task) {
      override def apply[A](fa: Log[Pure, A]): Task[A] = {
        val pure: Pure[A] = fa.value.run(LoggerFactory.getLogger("de.codecentric.MainInMemoryLogging"))
